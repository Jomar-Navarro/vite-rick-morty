...
<template>
	<div class="text-center my-3 container">
		<h1 class="text-white fw-bolder">{{ store.mainTitle }}</h1>
		<div class="d-flex my-5 justify-content-center">
			...
			<input
				v-model.trim="speciesToSearch"
				@keyup.enter="startSearch"
				class="form-control w-25"
				list="datalistOptionsSpecies"
				id="exampleDataList"
				placeholder="Type to search a species..."
			/>

			<datalist id="datalistOptionsSpecies">
				<option
					v-for="(character, index) in this.store.characterSpecies"
					:key="index"
					:value="character"
				></option>
			</datalist>
